...
可失敗性證據（ADR-0002 條件 1，可重跑）:
  正向  role_failure_watch.py --window 200               → 結束碼 0
  反向  role_failure_watch.py --window 200 --max 0.001   → 結束碼 1
  「有失敗分支」不等於「到得了失敗分支」—— 本檢查舊版即有 return 1，
  但因缺新鮮度檢查而結構上永遠走不到，故一律以實跑證明。
