...
> ⚠️ **排程更新（2026-09）**：正式環境（Linux · .166）已改為 **每日備份**（`deploy/crontab.txt` → `daily_mongodb_backup`，每日 01:00，保留 14 天），並搭配每日資料健康檢查與每週還原性驗證（`verify_backup.py`）。下方 macOS launchd 內容為開發機歷史設定，仅供參考。
...
### 備份策略
- **自動備份**（正式環境/Linux cron）: **每日凌晨 1:00** 自動執行
- **備份位置**: `~/Stock/mongodb_backups/`
- **保留策略**: 自動清理 14 天前的舊備份（每日頻率）
- **健康/驗證**: 每日 `data_health_check.py`（48h 新鮮度門檻）+ 每週 `verify_backup.py` 還原性驗證
- **備份格式**: tar.gz 壓縮檔案
- **預計大小**: 180-200 MB/個
