...
DATA_TABLES = {
    "技術面": [
        ...
        },
        {
            ...
            "disabled": True,
            "disabled_reason": "已由 twse_daily_update.py(TWSE OpenAPI/MI_INDEX, datetime新schema)每日寫入 + market_perdate_price.py 補洞取代;FinMind 此路徑寫 string-date 舊schema 會被 stock_price validator 全數拒絕(每小時空轉噴error/耗API),同 margin/securities_lending 處置(2026-07-30)",
        ...
        },
        {
            ...
            "disabled": True,
            "disabled_reason": "TaiwanVariousIndicators5Seconds 為盤中5秒指標(常回0/配額吃緊時402);market_statistics 凍結2026-02-11;真正 TAIEX 已由 macro_sync.sync_taiex 寫入 stock_price(symbol=TAIEX)取代(2026-08-01)",
    ...
    "籌碼面": [
        {
            ...
            "disabled": True,
            "disabled_reason": "已由 twse_openapi_sync(TWSE OpenAPI, datetime新schema)取代;FinMind寫string-date舊schema會汙染同表,見 scripts/migrate_margin_schema.py",
            "params": {"start_date": "2020-01-01"},
            "indexes": [("stock_id", ASCENDING), ("date", DESCENDING)],
            "unique_keys": ["stock_id", "date"],
            "needs_symbols": True,
