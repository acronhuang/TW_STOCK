#!/usr/bin/env python3
"""
P0: 價格欄位精度遷移 - 將所有數值欄位遷移到 Decimal128

執行方式:
    python3 src/migrations/p0_decimal_migration.py --dry-run  # 預覽
...
MIGRATION_SPECS = {
    "dividend_detail": {
        ...
        "description": "股利政策表 - 將所有金額欄位轉為 Decimal128"
    },
    "taiwan_stock_per": {
        "decimal_fields": [
            "PER",
        ...
        "description": "本益比資料表 - 將比率欄位轉為 Decimal128"
    },
    "month_revenue_detail": {
        "decimal_fields": [
            "revenue",
            "revenue_month",
            "revenue_year"
        ],
        "description": "月營收表 - 將營收欄位轉為 Decimal128"
    }
}


class DecimalMigrator:
    """Decimal128 遷移工具"""
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        log_file = log_dir / f"decimal_migration_{timestamp}.log"
    ...
    def _convert_to_decimal128(self, value):
        ...
        try:
            ...
            if isinstance(value, (int, float)):
                ...
                self.logger.warning(f"無法轉換類型: {type(value)}")
                return value
        ...
        for doc in cursor:
            ...
            for field in decimal_fields:
                if field in doc:
                    ...
                    # 如果不是 Decimal128，則需要轉換
                    if not isinstance(value, Decimal128):
                        converted = self._convert_to_decimal128(value)
                        if converted != value:
    ...
    def migrate_all(self, dry_run: bool = True) -> dict:
        """遷移所有集合"""
        self.logger.info("\n" + "="*80)
        self.logger.info("🚀 開始 P0 階段：Decimal128 精度遷移")
        self.logger.info("="*80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="P0: 將所有數值欄位遷移到 Decimal128"
    )
    ...
    # 執行遷移
    migrator = DecimalMigrator(args.mongo_uri, args.db_name)
